...
const GroupList = ({
  ...
  const handleVerifyPassword = async () => {
    try {
      const result = await verifyGroupPassword(
        selectedGroupState._id,
        password
      );
    ...
    <div className="overflow-y-auto">
          ...
          >
            ...
            {unreadGroupMessages?.[group._id] > 0 && (
              <span className="bg-red-600 text-white text-xs font-bold px-2 py-1 rounded-full min-w-[20px] text-center">
                {unreadGroupMessages[group._id] > 99
                  ? "99+"
            ...
            {isAuthorizedForGroup(group._id) &&
              !group.admins?.some((admin) => admin._id === user?.id) && (
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          {/* Modal Content */}
          <div
            className={`relative w-full max-w-md p-6 rounded-xl shadow-2xl transform transition-all 
            ${
              darkMode
