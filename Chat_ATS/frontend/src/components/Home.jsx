...
const Home = () => {
  ...
  useEffect(() => {
    ...
    socket.on('user_status_change', ({ userId, online }) => {
      // console.log('Status change:', userId, online); // For debugging
    });
  ...
  // Emit user connected when component mounts
  useEffect(() => {
    if (socket && user) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center">
      ...
      <button
        ...
        className="flex items-center gap-2 px-6 py-3 bg-white text-blue-600 font-semibold rounded-full shadow-lg hover:bg-gray-100 transition fixed bottom-6 right-6"
      >
